...
### 手順  
1. Aspera Serverのデスクトップ上で右クリックし**端末を開く**を選択します。

2. 下記コマンドを実行してインストールします。rootユーザーで実行します。  
    ```
    su - 
...
2. 下記のコマンドを実行するとAsperaのGUIコンソールが表示されます。  
   ```
   su -
   #rootのパスワード入力
...
4. 表示されたポップアップウインドウで**ライセンスファイルのインポート**をクリックし、ダウンロードしたライセンスファイルを指定して適用します。  
